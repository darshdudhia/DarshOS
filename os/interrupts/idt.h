...
#define INT_FLAGS      0x8E   //P = 1, DPL = 00, S = 0, Type 1110 (32bit interrupt gate)
...
{
    PrintString("DEFAULT EXCEPTION HANDLER - NO ERROR CODE");
}
...
{
    PrintString("DEFAULT EXCEPTION HANDLER - ERROR CODE: ");
    PrintString(HexToString(error_code));
}
...
void init_idt_32(void)
{
    ...
    for (uint8_t entry = 0; entry < 32; entry++) {
        if (entry == 8  || entry == 10 || entry == 11 || entry == 12 || 
            ...
            set_idt_descriptor_32(entry, (void*)default_excp_handler_err_code, TRAP_GATE_FLAGS);
        } else {
            // Exception does not take an error code
            set_idt_descriptor_32(entry, (void*)default_excp_handler, TRAP_GATE_FLAGS);
        }
    }

    // Set up regular interrupts (ISRs 32-255)
    for (uint16_t entry = 32; entry < 256; entry++) {
        set_idt_descriptor_32(entry, (void*)default_int_handler, INT_FLAGS);
    }
